...
import { QuDetail } from './qu-detail.entity';

@Entity('qu_head')
export class QuHead {
  @PrimaryGeneratedColumn({ name: 'qu_id' })
  quId: number;

  @Column({ name: 'qu_no', length: 50, unique: true })
  quNo: string;

  @Column({ name: 'qu_date', type: 'date', default: () => 'CURRENT_DATE' })
  quDate: Date;
  ...
  @OneToMany(() => QuDetail, detail => detail.quHead, { cascade: true })
  details: QuDetail[];
}
